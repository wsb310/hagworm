...
class RequestBaseHandler(RequestHandler, _BaseHandlerMixin):
    ...
    def prepare(self):
        ...
        if self._prepare_funcs:
            for func in self._prepare_funcs:
                result = func()
    ...
    def on_finish(self):
        ...
        if self._closure_funcs:
            for func in self._closure_funcs:
                func()
...
class DownloadAgent(RequestBaseHandler, DownloadBuffer):
    ...
    @coroutine
    def _fetch_data(self, url, params, cookies, headers):

        yield self.fetch(url, params, cookies, headers)
    ...
    @coroutine
    def transmit(self, url, file_name=None, *, params=None, cookies=None, headers=None):
        ...
        try:
            ...
            self._fetch_data(url, params, cookies, headers)

            yield self._flush_data()
