...
class MySQLDelegate:
    ...
    async def get_db_client(self, readonly=False, *, alone=False):
...
class DBClient(_ClientBase, AsyncContextManager):
    ...
    async def _close(self, destroy=False):

        if self._is_active:

            self._is_active = False

            await self._close_conn(destroy or self._running)
    ...
    async def _close_conn(self, destroy=False):
